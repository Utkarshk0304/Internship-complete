...
const Suit = (props) => {
  ...
  const renderSelectedImages = () => {
    if (props.subSubButtonId !== null && !props.waistcoat && !props.extraTrousers ) {
    ...
    if (props.subSubButtonId !== null&& !props.waistcoat  && props.extraTrousers ) {
      const selectedButton =
        props.MainArray[props.ids - 1].ExtraTrousers[subButtonId - 1].Suboptions[
          props.subSubButtonId - 1
        ].renderLayers;
      // Update the selected images for the current subButton in refs
      selectedImagesRefs.current[subButtonId] = selectedButton;
  ...
  return (<React.Fragment>
    ...
    >
      ...
      { updatedSubButtonData.map((items)=>
